...
@Validated
@AllArgsConstructor
public class SaleAddWholesaleUseCase implements Function<Flux<InventoryDTO>, Flux<ProductDTO>> {
    ...
    private Mono<InventoryDTO> checkQuantityMinimum(InventoryDTO inventoryDTO) {
        ...
        if (saleQuantity < minimum) {
            ...
            eventBus.publishError("Validation Error in Wholesale: ", error.getMessage());
        ...
        return mongoTemplate.findById(id, Product.class)
            .switchIfEmpty(
                Mono.defer(() -> {
                    ...
                    eventBus.publishError("Product Error in Wholesale: ", error.getMessage());
                    return Mono.error(error);
                })
            )
            .map(productFound -> {
    ...
    private Mono<ProductDTO> changeProduct(InventoryDTO inventoryDTO) {
        ...
        return mongoTemplate.save(product)
            .doOnError(error -> eventBus.publishError("Save Error in Wholesale: ", error.getMessage()))
            .doOnSuccess(success -> {
                eventBus.publishRetailSale("Wholesale Recorded: ", inventoryDTO);
                eventBus.publishProductMovement("Product Sold in Wholesale: ", inventoryDTO);
            })
